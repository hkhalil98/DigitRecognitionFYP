...
# from scipy.spatial.distance import cdist NOT IMPORTING???
...
class GraphNet(nn.Module):
    ...
    def __init__(self, image_size = 28, pred_edge = False):
        ...
        if pred_edge:
            ...
            col, row = np.meshgrid(np.arange(image_size), np.arange(image_size)) # (28 x 28) 
            coord = np.stack((col, row), axis = 2).reshape(-1, 2)  # (784 x 2)
            coord_normalized = (coord - np.mean(coord, axis = 0)) / (np.std(coord, axis = 0) + 1e-5) # Normalise matrix
    ...
    def precompute_adjacency_images(image_size):
        ...
        col, row = np.meshgrid(np.arange(image_size), np.arange(image_size)) # (28 x 28) 
        coord = np.stack((col, row), axis = 2).reshape(-1, 2) / image_size # (784 x 2) --> normalise
